...
- `app/clientauth.py` — accessi console CLIENTE gestiti da FORMA: email+password al primo accesso poi codice a 6 cifre (solo FORMA lo genera), chiave tenant server-side cifrata, sessioni HMAC in cookie HttpOnly, ghost FORMA→pannello cliente; endpoint `/client/*` + `/admin/clients*`; DDL `db/ovyon_client_access.sql`; fail-closed senza `CLIENT_SESSION_SECRET`; login cliente su `/panel/#cliente`
...
- `app/main.py` — API: `/health` `/ingest` `/chat` (con `{"stream": true}` SSE) `/upload`, e per il connettore MCP `/search` `/document` `/context` `/writeback`; admin: `/admin/learning` (task di apprendimento da gap+👎), `/admin/roadmap` (roadmap verso l'«AI OS», benchmark Zoey OS — dati in `app/roadmap.py`, confronto in `docs/confronto-divina-zoey.md`), `/admin/tasks` (coda task persistente del cervello — `app/braintasks.py`, DDL `db/ovyon_tasks.sql`), `/admin/brain`, `/admin/brain/notes` e `/admin/brain/graph` (cervello vivo in console: KPI vault, note recenti, ricerca metadati da `documents`, grafo reale dei `[[link]]` rigenerato a ogni ingest — `app/brain.py`, DDL `db/ovyon_graph.sql`), `/admin/proposals` (proposte di auto-miglioramento audit→owner, PRIVATE: solo ADMIN_TOKEN — `app/proposals.py`; approvazione → brain_tasks), quote per tenant `quota_day` E `quota_month` (mensile; su Supabase via chiave `quota_month` nel jsonb `branding`)
- `widget/voce.js` — **MOTORE VOCALE UNICO** (U1): frasi, coda TTS, barge-in, invio automatico a fine parlato, parziali STT, ampiezza per l'orb. Usato dal widget (caricato accanto a `embed.js`) e dalla console (`panel/voce.js` = copia byte-identica, parità garantita da `tests/test_voce_sentenze.py`; chunker testato da `scripts/test_voce_sentenze.js`). La meccanica voce si modifica SOLO qui (+ `cp` verso panel/ e i due repo).
