...
> Leggi questo file prima di lavorare. Aggiornato al 03-08-2026 sera (V13: la
> prima porta — da sei campi a uno, si guarda Divina prima di collegarla, la
> chiave del cliente nasce sul server). Prodotto: **Divina** — dominio
...
- `app/autodoc.py` — V11/D: **Divina sa come funziona, e lo sa dal vault.** Le note di `ovyon/divina/` lette dal DISCO (non dall'indice: `ovyon` è uno scope, e allargare il filtro per una funzione di documentazione sposterebbe una decisione di sicurezza dentro un problema di prodotto). Cartella COSTANTE, mai un percorso dalla richiesta. `contesto()` risponde citando le note, prima di `systemq`; `punto_del_lavoro()` (D2) dice a che punto è la KB di un cliente, con la conseguenza e non solo il buco
...
**V11 · La regola che resta (02-08 sera tardi).**
9. **Ogni giro toglie qualcosa.** Una sezione che nessuno apre da due settimane non è una funzione in attesa: è un costo che si paga ogni volta che si cerca qualcos'altro, e una superficie in più da difendere. La barra aveva diciotto destinazioni e in due giorni ne erano state aperte cinque — la causa era nostra, perché nessun giro aveva mai tolto niente. Il criterio: **una sezione esiste se un imprenditore appena partito, che non sa niente di AI, capisce in tre secondi perché aprirla.** Se un giro non ha niente da togliere, si toglie il giro.
10. **Ogni rotta dichiara chi la chiama.** Il contratto va in tutte e due le direzioni: `scripts/contract_console.py` → `orfane()` + `CHIAMANTI_FUORI`, e una rotta nuova senza dichiarazione fa fallire la CI.

**V12 · Le due regole del 3/08, nate da difetti che 691 test non vedevano.**
11. **Se il guardiano non la preme, l'azione non è provata.** Le azioni della home erano `<span class="pill">`: sembravano bottoni e non facevano niente, e il controllo del V11 verificava che la home DICESSE cosa fare, non che il rimedio funzionasse. Adesso il guardiano preme e registra l'esito di ognuno (chiama l'API · apre una finestra · cambia pagina · dice perché non può); «niente» fa fallire la CI. Vale per ogni azione nuova.
12. **Una task si chiude quando una prova la percorre**, non quando il codice c'è. Dove la prova non esiste la task resta «da verificare» e il motivo è scritto — e le prove su dati di demo provano il cablaggio, non la produzione: la distinzione va detta, non sottintesa.

**V13 · La regola del 3/08 sera.**
13. **La prima porta si conta, non si giudica.** Chiedeva sei campi, due dei quali segreti, prima di far vedere qualunque cosa — l'unica porta che il criterio del V11 non aveva mai incontrato. Adesso ne chiede uno, il guardiano li **conta** a ogni push, e il percorso è «guardo → capisco → collego» invece di «muro → sei campi → forse». Corollario: **un segreto non si fa passare da un modulo all'altro.** Creare un accesso cliente conia la chiave sul server e non la mostra a nessuno — meno passaggi e meno superficie sono la stessa cosa.
...
- ✅ V13 (03-08 sera, in PR): **la prima porta** — da sei campi a uno, con i cinque avanzati dietro un cassetto chiuso e un bottone per **guardare** Divina prima di collegarla; **creare un accesso cliente non passa più da una chiave incollata** (nasce sul server, legata a uno scope, e non torna indietro).
- ✅ V12 (03-08, in PR): **provare quello che non si vede** — i rimedi della home diventano bottoni veri e **il guardiano li PREME**; la domanda di seguito trova la scheda e non la KB pubblica (l'espansione non partiva: `_ANAFORA` non riconosceva le domande che cominciano con una parola interrogativa); il guardiano entra come cliente, affida un lavoro a un agente, spegne una dipendenza apposta e cammina la registrazione fino in fondo. Nove task in «da verificare» hanno adesso una prova che le percorre.
- ✅ V11 (02-08 sera tardi, in PR): **il giro che toglie** — sei destinazioni invece di diciotto, «Amministrazione» owner-only e chiusa, «Human · evoluzione» fuori del tutto (anche per sicurezza: dati sanitari + motore in US West); **la home dice tre cose da fare oggi** con la ragione, al posto di settanta righe di task; **il cliente si registra come azienda** e il sito fa partire la KB; **Divina si documenta nel vault** e cita le proprie note.
...
- V13 (la prima porta: da sei campi a uno, e la chiave che non passa dagli appunti): `docs/la-prima-porta.md`
- V12 (i pulsanti che non erano pulsanti + il filo che pescava la KB + il guardiano che preme): `docs/provare-quello-che-non-si-vede.md`
- V11 (il giro che toglie: sei porte, tre cose da fare, Divina che si documenta): `docs/il-giro-che-toglie.md`
