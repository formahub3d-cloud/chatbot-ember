...
> Leggi questo file prima di lavorare. Aggiornato al 02-08-2026, pomeriggio (V9:
> il degrado dichiarato dove si usa, la KB del cliente che nasce dal suo sito, le
> capacità raggiungibili parlando, la conversazione che dura). Prodotto: **Divina** — dominio
...
- `app/degrado.py` — V9/A: **una funzione spenta lo dice DOVE si usa.** Registro funzione→dipendenze (tabelle via `dbcheck`, variabili via `settings`) con tre esiti mai confusi: acceso · spento · non-so. Il «cosa smette di funzionare» si legge da `dbcheck.ATTESE`, mai riscritto. La console lo disegna con `boxDegrado()`/`vuotoOnesto()` — al cliente niente riga tecnica
