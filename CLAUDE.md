...
- `app/main.py` — API: `/health` `/ingest` `/chat` (con `{"stream": true}` SSE) `/upload`, e per il connettore MCP `/search` `/document` `/context` `/writeback`; admin: `/admin/learning` (task di apprendimento da gap+👎), `/admin/roadmap` (roadmap verso l'«AI OS», benchmark Zoey OS — dati in `app/roadmap.py`, confronto in `docs/confronto-divina-zoey.md`), `/admin/tasks` (coda task persistente del cervello — `app/braintasks.py`, DDL `db/ovyon_tasks.sql`), `/admin/brain` e `/admin/brain/notes` (cervello vivo in console: KPI vault, note recenti, ricerca metadati da `documents` — `app/brain.py`), `/admin/proposals` (proposte di auto-miglioramento audit→owner, PRIVATE: solo ADMIN_TOKEN — `app/proposals.py`; approvazione → brain_tasks), quote per tenant `quota_day` E `quota_month` (mensile; su Supabase via chiave `quota_month` nel jsonb `branding`)
